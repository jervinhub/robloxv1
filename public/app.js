...
async function loadLimitedRobloxItems(){
 try{
 const r=await fetch('/api/limited-items');
 ...
 function render(){
  ...
  const nm=document.getElementById('limitedName');
  if(nm) nm.textContent=item.name;
 }
 if(items.length){
  render();
  setInterval(()=>{idx=(idx+1)%items.length;render();},5000);
 }
 }catch(e){console.error(e);}
}
document.addEventListener('DOMContentLoaded',loadLimitedRobloxItems);
